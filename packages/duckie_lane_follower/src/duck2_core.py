"""Shared autonomy engine with injected transport and clocks; no ROS imports."""
import os
import copy
import json
import math
...
class Observation:
    """Immutable decision inputs; timestamps retain source and receiver meaning."""
    source_stamp: float
    received_at: float
    lane_error: Optional[float]
    lane_limits: Optional[Tuple]
    lane_both_visible: bool
    lane_diagnostic: str
    duck_boxes: Tuple
    road_geometry: Optional[Tuple]
    red_visible: bool
    obstacle_box: Optional[Tuple]
    passing_lane_box: Optional[Tuple]
...
class AutonomyCore:
    def __init__(self, node_name, runtime, clock=time):
        ...
        self.bridge = self.runtime.CvBridge()

        self.publisher = self.runtime.Publisher(
            self.wheels_topic,
            self.runtime.WheelsCmdStamped,
            queue_size=1,
        )

        # Driving parameters. Keep these conservative for first tests.
        self.drive_enabled = self.runtime.get_param("~drive_enabled", False)
        self.show_debug = self.runtime.get_param("~show_debug", False)
        self.flip_steering = self.runtime.get_param("~flip_steering", True)
        ...
        self._last_control_time = self.clock.monotonic()
        self._last_publish_time = self.clock.monotonic()
        self._last_wheel_speeds = (0.0, 0.0)

        self.route_enabled = self.runtime.get_param("~route_enabled", False)
        self.junctions_calibrated = self.runtime.get_param("~junctions_calibrated", False)
        self.route = validate_route(self.runtime.get_param("~route", ["A", "D", "C", "E", "A"]))
        self.route_index = 1
        self.auto_continue = self.runtime.get_param("~auto_continue", True)
        ...
        self.require_client_heartbeat = self.runtime.get_param("~require_client_heartbeat", False)
        ...
        self.obstacle_enabled = self.runtime.get_param("~obstacle_enabled", True)
        ...
        self.avoidance_enabled = self.runtime.get_param("~avoidance_enabled", False)
        self.avoidance_calibrated = self.runtime.get_param("~avoidance_calibrated", False)
        ...
        self.status_publisher = self.runtime.Publisher(
            f"/{self.vehicle_name}/lane_follower/status", self.runtime.String, queue_size=1, latch=True
        )
        ...
        self._last_frame_time = self.clock.monotonic()
        self._camera_timeout = 0.5
        self.subscriber = self.runtime.Subscriber(
            self.camera_topic,
            self.runtime.CompressedImage,
            self.callback,
            queue_size=1,
            buff_size=2 ** 24,
        )

        self.runtime.on_shutdown(self.on_shutdown)
        self.command_subscriber = self.runtime.Subscriber(
            f"/{self.vehicle_name}/lane_follower/command", self.runtime.String,
            self.command_callback, queue_size=10,
        )
        self._camera_watchdog = self.runtime.Timer(
            self.runtime.Duration(0.1), self.check_camera_timeout
        )

        self.runtime.loginfo(f"LaneFollowerNode started for {self.vehicle_name}")
        self.runtime.loginfo(f"Camera topic: {self.camera_topic}")
        self.runtime.loginfo(f"Wheels topic: {self.wheels_topic}")
        self.runtime.loginfo(f"drive_enabled={self.drive_enabled}, show_debug={self.show_debug}")

    def number_param(self, name, default):
        value = self.runtime.get_param(name, default)
        if isinstance(value, bool) or not isinstance(value, (int, float)) or not math.isfinite(value):
            raise ValueError("%s must be a finite number" % name)
        return float(value)

    def topic_param(self, name, default):
        value = self.runtime.get_param(name, default)
    ...
    def color_range(self, name, lower, upper):
        bounds = [self.runtime.get_param("~%s_%s" % (name, suffix), default)
    ...
    def update_obstacle(self, box):
        ...
        if box is not None:
            ...
            if not self.obstacle_stop_latched:
                self._control_epoch += 1
                self.runtime.loginfo("Obstacle candidate in lane; stop requires explicit Continue")
            self.obstacle_stop_latched = True
            self.manual_stop = True
            ...
            self._obstacle_clear_since = self.clock.monotonic()
    ...
    def publish_wheels(self, left_speed, right_speed):
        with self._wheel_lock:
            ...
            if not self._camera_valid or self.clock.monotonic() - self._last_frame_time > self._camera_timeout:
                left_speed = right_speed = 0.0
            ...
            msg = self.runtime.WheelsCmdStamped()
            msg.header.stamp = self.runtime.Time.now()
    ...
    def status(self):
        return {
            ...
            "camera_age": max(0.0, self.clock.monotonic() - self._last_frame_time),
    ...
    def publish_status(self):
        self.status_publisher.publish(self.runtime.String(data=json.dumps(self.status(), allow_nan=False)))
    ...
    def client_expired(self):
        ...
        return self.clock.monotonic() - last_seen > self._client_timeout
    ...
    def check_camera_timeout(self, event):
        with self._wheel_lock:
            ...
            if self.clock.monotonic() - self._last_frame_time > self._camera_timeout:
                self._camera_valid = False
            ...
            try:
                ...
                if action != "stop":
                    ...
                    if (isinstance(issued, bool) or not isinstance(issued, (float, int))
                            or not math.isfinite(issued) or not -2 <= self.clock.time() - issued <= 5):
                ...
                if action == "heartbeat":
                    ...
                    if issued > previous:
                        self._heartbeat_clients[client_id] = (issued, self.clock.monotonic() - max(0.0, age))
                ...
                if action != "stop":
                    if client_id is not None:
                        ...
                        if (self.clock.monotonic() - self._heartbeat_clients.get(client_id, (0, -float("inf")))[1]
                                > 1.5):
                ...
                if action == "stop":
                    ...
                    if (self.obstacle_stop_latched and (
                            ...
                            or self.clock.monotonic() - self._obstacle_clear_since < 0.5)):
    ...
    def begin_crossing(self):
        ...
        if self._stop_started is None or self.clock.monotonic() - self._stop_started < self.stop_hold_seconds:
        ...
        self._crossing_started = self.clock.monotonic()
    ...
    def navigation_wheels(self, lane_error, red_visible):
        ...
        if red_visible and not self.red_stop_latched:
            ...
            self.runtime.loginfo("Red stop line detected; holding position")
        return self.compute_wheel_speeds(lane_error)
    ...
    def camera_stamp(self, msg, received_at):
        ...
        now = self.runtime.Time.now().to_sec()
        ...
        if self.clock.monotonic() - received_at > self._camera_timeout:
    ...
    def observe_bgr(self, image, source_stamp, received_at):
        """Compute on a private snapshot, with no transport calls or state commit."""
        self.validate_image(image)
        perception = copy.copy(self)
        perception._duck_boxes = perception.detect_ducks_bgr(image) if perception.obstacle_enabled else []
        perception._road_geometry = perception.detect_road_geometry(image)
        red_visible = perception.detect_red_stop(image)
        lane_error, debug_image, debug_mask = perception.detect_lane_bgr(image)
        obstacle_box = perception.detect_obstacle_bgr(image)
        ...
        if perception._road_geometry is not None:
            ...
            passing_lane_box = perception.detect_obstacle_bgr(image)
            perception._lane_limits = limits
        frozen = lambda value: None if value is None else tuple(value)
        observation = Observation(source_stamp, received_at, lane_error,
            frozen(perception._lane_limits), perception._lane_both_visible, perception._lane_diagnostic,
            tuple(tuple(box) for box in perception._duck_boxes), frozen(perception._road_geometry),
            red_visible, frozen(obstacle_box), frozen(passing_lane_box))
        return observation, debug_image, debug_mask
    ...
    def callback(self, msg):
        ...
        with self._camera_lock:
            ...
            try:
                ...
                observation, debug_image, debug_mask = self.observe_bgr(img_bgr, stamp, received_at)
                lane_error = observation.lane_error
                red_visible = observation.red_visible
                obstacle_box = observation.obstacle_box
                passing_lane_box = observation.passing_lane_box
                with self._wheel_lock:
                    if self._stopping:
                        return
                    ...
                    self._last_frame_time = self.clock.monotonic() - max(
                        self.clock.monotonic() - received_at, self.runtime.Time.now().to_sec() - stamp, 0.0)
                    self._last_camera_stamp = stamp
                    self._camera_valid = True
                    self._camera_error = None
                    self._last_lane_error = lane_error
                    self._lane_limits = observation.lane_limits
                    self._lane_both_visible = observation.lane_both_visible
                    self._lane_diagnostic = observation.lane_diagnostic
                    self._duck_boxes = list(observation.duck_boxes)
                    self._road_geometry = observation.road_geometry
                    self.check_client_connection()
            ...
            except (self.runtime.CvBridgeError, cv2.error, ValueError, TypeError, AttributeError) as error:
                self.runtime.logwarn("Could not process camera image: %s", error)
    ...
    def on_shutdown(self):
        ...
        self.runtime.loginfo("LaneFollowerNode stopped; wheels set to zero")
