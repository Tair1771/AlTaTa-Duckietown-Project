# Historical desktop applications

These are retained for reference and regression checks. Current operation uses
`Start-Duck2Companion.cmd` and local live chat; no API key is required.
The legacy API experiment is not the current chatbot or evidence of its training.

## Earlier connected companion (not needed for the current project workflow)

Run `py -3 laptop/duck2_chat.py` from the repository root with Python 3 and Tkinter on Windows. It uses only standard
